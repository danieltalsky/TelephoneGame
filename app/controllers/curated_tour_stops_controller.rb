class CuratedTourStopsController < ApplicationController
  ...
  # GET /curated_tour_stops/new
  def new
    @curated_tour_stop = CuratedTourStop.new
  end

  # GET /curated_tour_stops/1/edit
  def edit
  end

  # POST /curated_tour_stops
  def create
    @curated_tour_stop = CuratedTourStop.new(curated_tour_stop_params)

    if @curated_tour_stop.save
      redirect_to @curated_tour_stop, notice: 'Curated tour stop was successfully created.'
    else
      render action: 'new'
    end
  end

  # PATCH/PUT /curated_tour_stops/1
  def update
    if @curated_tour_stop.update(curated_tour_stop_params)
      redirect_to @curated_tour_stop, notice: 'Curated tour stop was successfully updated.'
    else
      render action: 'edit'
    end
  end

  # DELETE /curated_tour_stops/1
  def destroy
    @curated_tour_stop.destroy
    redirect_to curated_tour_stops_url, notice: 'Curated tour stop was successfully destroyed.'
  end
  ...
  private
    ...
    def curated_tour_stop_params
      params[:curated_tour_stop].permit(:work_id, :curated_tour_id, :caption_text, :sequential_id)
    end
end
